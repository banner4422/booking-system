        ...
        <div className={frontpage ? "relative z-10 bg-gray-800 rounded-lg" : "relative z-10 bg-gray-900 rounded-lg"}>
          ...
          <div className="px-14 pb-4">
              ...
              <a className="inline-block pt-4 text-2xl text-white hover:text-gray-100 font-bold border-t border-lime-400" href={`events/${props.id}`}>{props.name}</a>
                        ...
                        <a className="px-14 pb-5 inline-block text-sm text-gray-300">{`${capitalizeFirstLetter(DateTime.fromJSDate(new Date(props.dateStart)).setLocale('da').toLocal().toFormat('DDDD'))} - ${capitalizeFirstLetter(DateTime.fromJSDate(new Date(props.dateEnd)).setLocale('da').toLocal().toFormat('DDDD'))}`}</a>
